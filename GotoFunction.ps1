...
function _goto_print_similar {
    param([string]$input)
    # Filter aliases to include only those that contain the input string case-insensitively
    ...
    elseif ($similar.Count -gt 1) {
        # Sort aliases by relevance (e.g., start of the name or shortest match)
        $sortedSimilar = $similar | Sort-Object { $_.IndexOf($input, [StringComparison]::OrdinalIgnoreCase) }
